...
export default function NoticeCard({ notice }) {
  const pdf = notice.pdf || notice.file;
  ...
  return (
    <article className="flex flex-col gap-4 rounded-card border border-slate-200 bg-white p-5 shadow-soft sm:flex-row sm:items-center sm:justify-between">
      <div className="flex gap-4">
        ...
        <div>
          <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-secondary">{[notice.category, notice.date].filter(Boolean).join(' • ')}</p>
          <h3 className="mt-1 font-heading text-xl font-bold text-dark">{notice.title}</h3>
        </div>
      </div>
      {pdf && (
        <a href={pdf} className="focus-ring inline-flex items-center justify-center gap-2 rounded-lg bg-dark px-4 py-3 text-sm font-bold text-white">
