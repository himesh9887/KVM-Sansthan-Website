...
export default function Notice() {
  ...
  const filtered = useMemo(() => notices.filter((notice) => (category === 'All' || notice.category === category) && notice.title.toLowerCase().includes(query.toLowerCase())), [query, category]);
  return (
    <>
      ...
      <section className="section-pad bg-background">
        <Container>
          ...
          <div className="space-y-4">{filtered.map((notice) => <NoticeCard key={notice.title} notice={notice} />)}</div>
        </Container>
      </section>
    </>
  );
